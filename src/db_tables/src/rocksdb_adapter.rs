use crate::primitives::{
    base::DEFAULT_VRRB_DB_PATH,
    environment::{get_vrrb_environment, Environment},
};
...
impl RocksDbAdapter {
    pub fn new(path: std::path::PathBuf, column_family: &str) -> StorageResult<Self> {
...
impl VersionedDatabase for RocksDbAdapter {
    ...
    fn value_history(
        &self,
    ) -> HashMap<patriecia::KeyHash, Vec<(patriecia::Version, Option<patriecia::OwnedValue>)>> {
        todo!()
    }
}
